...
plt.plot(sig)
plt.subplot(2,1,2)
plt.imshow(ex1,extent=[-1,1,800,1200], cmap='PRGn',aspect='auto',vmax=abs(ex1).max(),vmin=-abs(ex1).max())
plt.show(block=False)


#wavelet cwt ex2
#read the target signal from the wav file.
sig, sr=librosa.load("ex2.wav")
print(len(sig), sr)
...
plt.plot(sig)
plt.subplot(2,1,2)
plt.imshow(ex2,extent=[-1,1,800,1200], cmap='PRGn',aspect='auto',vmax=abs(ex2).max(),vmin=-abs(ex2).max())
plt.show(block=False)
...
plt.plot(sig)
plt.subplot(2,1,2)
plt.imshow(ex3,extent=[-1,1,800,1200], cmap='PRGn',aspect='auto',vmax=abs(ex3).max(),vmin=-abs(ex3).max())
plt.show(block=True)
